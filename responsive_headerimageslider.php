 ...
 * Description: A simple Responsive header image slider. Also work with Gutenberg shortcode block.
 * Version: 3.2.1
...
if( !defined( 'SP_RHIMGS_VERSION' ) ) {
	define( 'SP_RHIMGS_VERSION', '3.2.1' ); // Version of plugin
}
...
function responsive_slider_rewrite_flush() {  
		...
		if( is_plugin_active('wp-slick-slider-and-image-carousel-pro/wp-slick-image-slider.php') ){
			 add_action('update_option_active_plugins', 'free_responsive_slider_deactivate_premium_version');
		}

}

function free_responsive_slider_deactivate_premium_version(){
...
add_action( 'admin_notices', 'free_responsive_slider_admin_notice');

function free_responsive_slider_admin_notice() {
	global $pagenow;

	$dir = WP_PLUGIN_DIR . '/wp-slick-slider-and-image-carousel-pro/wp-slick-image-slider.php';
	$notice_link        = add_query_arg( array('message' => 'rhimgs-plugin-notice'), admin_url('plugins.php') );
	$notice_transient   = get_transient( 'rhimgs_install_notice' );

	if( $notice_transient == false && $pagenow == 'plugins.php' && file_exists( $dir ) && current_user_can( 'install_plugins' ) ) {        
		echo '<div class="updated notice" style="position:relative;">
					<p>
						<strong>'.sprintf( __('Thank you for activating %s', 'responsive-header-image-slider'), 'WP Responsive header image slider').'</strong>.<br/>
						'.sprintf( __('It looks like you had PRO version %s of this plugin activated. To avoid conflicts the extra version has been deactivated and we recommend you delete it.', 'responsive-header-image-slider'), '<strong>(<em>WP Slick Slider and Image Carousel Pro</em>)</strong>' ).'
					</p>
...
//include admin class
include_once( SP_RHIMGS_DIR . '/admin/class-rhimgs-admin.php' );
